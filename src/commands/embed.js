...
// Chemin corrigé : remonter 2 niveaux depuis src/commands/ pour atteindre la racine
...
async function sendDashboard(interaction, config, messageTitle = "Configuration de votre Embed Sandwich") {
    ...
    const row2 = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder()
            ...
            .setLabel(config.isEditing ? 'Mettre à jour le Live 💾' : 'Déployer le Sandwich 🚀')
...
function buildSandwichPayloads(config) {
    ...
    if (config.bottom.footerText) {
        embedBottom.setFooter({
            ...
            iconURL: (config.bottom.footerIcon && config.bottom.footerIcon.startsWith('http')) ? config.bottom.footerIcon : undefined
        });
    }
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcmd =>
            subcmd.setName('create').setDescription('Ouvrir le panneau de création du Sandwich')
        )
        .addSubcommand(subcmd =>
            subcmd.setName('edit')
                .setDescription('Modifier un pack Sandwich existant')
                .addStringOption(opt => opt.setName('message_id').setDescription('ID de l\'un des messages du pack').setRequired(true))
        )
        .addSubcommand(subcmd =>
            subcmd.setName('save')
    ...
    async execute(interaction) {
        if (interaction.user.id !== interaction.guild.ownerId) {
            return interaction.reply({
                content: '❌ **Sécurité :** Seul le propriétaire du serveur peut utiliser cette commande.',
                ephemeral: true
            });
        }
        ...
        if (subcommand === 'edit') {
            ...
            if (!targetPack) {
                return interaction.reply({ content: '❌ Aucun pack Sandwich actif trouvé avec cet ID.', ephemeral: true });
            }
            ...
            return sendDashboard(interaction, editConfig, "Mode Édition en Live Activé");
        }
        ...
        if (subcommand === 'save') {
            ...
            saveData(data);
            return interaction.reply({ content: `💾 Template \`${templateName}\` sauvegardé !`, ephemeral: true });
        }
        ...
        if (subcommand === 'load') {
            ...
            return interaction.reply({ content: `🔄 Template \`${templateName}\` chargé. Fais \`/embed create\` pour le voir.`, ephemeral: true });
        }
    }
};
...
module.exports.handleInteraction = async (interaction) => {
    ...
    try {
        if (interaction.isButton()) {
            ...
            if (interaction.customId === 'sw_edit_top') {
                ...
                modal.addComponents(
                    ...
                    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('color').setLabel('Couleur (Hexadécimal, ex: #e67e22)').setStyle(TextInputStyle.Short).setValue(currentConfig.top.color).setRequired(false)),
            ...
            if (interaction.customId === 'sw_manage_buttons') {
                if (currentConfig.middle.buttons.length >= 5) {
                    ...
                    return interaction.reply({ content: "❌ Limite de 5 boutons atteinte.", components: [resetRow], ephemeral: true });
                }
            ...
            if (interaction.customId === 'sw_preview') {
                ...
                await interaction.followUp({ content: "👀 **[APERÇU OWNER]** Layout Sandwich simulé :", ephemeral: true });
            ...
            if (interaction.customId === 'sw_deploy_edit') {
                ...
                if (!channel) return interaction.followUp({ content: "❌ Salon cible introuvable.", ephemeral: true });
                ...
                return interaction.followUp({ content: "✅ Pack Sandwich mis à jour en direct !", ephemeral: true });
            }
        }
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (interaction.customId === 'sw_modal_deploy_target') {
                ...
                if (!channel || !channel.isTextBased()) {
                    return interaction.followUp({ content: "❌ Salon introuvable. Vérifie l'ID.", ephemeral: true });
                }
                ...
                activeBuilders.set(interaction.user.id, createDefaultConfig());
                return interaction.followUp({ content: `🚀 Pack Sandwich déployé dans <#${channel.id}> !`, ephemeral: true });
            }
        }
        ...
        try {
            if (interaction.replied || interaction.deferred) {
                await interaction.followUp({ content: "❌ Une erreur est survenue. Consulte la console.", ephemeral: true });
            } else {
                await interaction.reply({ content: "❌ Une erreur est survenue. Consulte la console.", ephemeral: true });
            }
        } catch (e) {}
    }
};
